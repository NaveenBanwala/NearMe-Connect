...
export default function BlockEditor() {
  ...
  // ✅ FIX: Points from BoundaryDrawer are already real lat/lng objects — use them directly.
  // No broken percentage-to-coordinate conversion needed.
  const handleSaveBoundary = async (points) => {
    if (!selected) return
    setWorking(true)
    try {
      // points = [{ lat, lng }, ...] — directly from Leaflet clicks
      const coords = points.map(p => [p.lng, p.lat]) // GeoJSON is [lng, lat]
  ...
  // ✅ FIX: Build a proper cluster object from the selected block so BoundaryDrawer
  // knows where to center the map AND can show the suggested boundary ring.
  const clusterForDrawer = selected
    ? {
        centerLat:    selected.center_lat,
        centerLng:    selected.center_lng,
        // Use saved radius if available, otherwise default to 300 m
        radiusMeters: selected.radius_meters ?? 300,
        displayName:  selected.name,
        heatLevel:    heatKey(selected.heat_score ?? 0),
      }
    : null
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1 space-y-3">
          ...
          <p className="text-xs text-slate-500 font-mono px-1">
            {blocks.length} block{blocks.length !== 1 ? 's' : ''} · approve a flagged cluster to create new blocks
          </p>
            ...
            {filtered.map(block => {
              ...
              return (
                <div
                  ...
                  className={clsx(
                    'card-hover p-3 cursor-pointer transition-all',
                    selected?.block_id === block.block_id && 'border-amber-500/40 bg-base-700'
                  )}
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-1">
                      <span className={clsx('badge border text-[10px]', hc.bg, hc.border, hc.text)}>
                        {heatLabel(block.heat_score || 0)}
                      </span>
                      <button
                  ...
                  {/* ✅ Show a small "boundary set" indicator */}
                  {block.boundary_geo_json && (
                    <p className="text-[10px] text-green-400 font-mono mt-1">✓ boundary set</p>
                  )}
                </div>
              )
            })}
        ...
        <div className="lg:col-span-2">
          {selected ? (
            <div className="card p-5 space-y-4">
              ...
              {/* ✅ FIX: Pass cluster so the map centers on the block and shows suggested boundary.
                  Also pass any existing saved boundary as initialPoints so admin can see + edit it. */}
              <BoundaryDrawer
                key={selected.block_id}           // remount when block changes
                clusterName={selected.name}
                cluster={clusterForDrawer}
                initialGeoJson={selected.boundary_geo_json ?? null}
            ...
            <div className="card h-full flex items-center justify-center py-24">
              <div className="text-center">
                ...
                <p className="text-slate-600 text-xs mt-1">
                  Approve vote clusters in Block Requests to create new blocks
                </p>
              </div>
            </div>
          )}
